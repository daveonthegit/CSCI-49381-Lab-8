...
def parse_log_file(logfile_path, legit_ip=LEGIT_IP, attack_ip=ATTACK_IP):
                        ...
                        if packet_type:
                            ...
                            if packet_type == "SYN":
                                ...
                                if conn_key not in connections:
                                    ...
                                    # This is a SYN retransmission - but don't count as unsuccessful yet
                                    if not connections[conn_key]["established"]:
                                        connections[conn_key]["syn_count"] += 1
                                        connections[conn_key]["syn_times"].append(time)
                                        # Remove immediate unsuccessful classification here
                            ...
                            # Connection termination or reset
                            elif packet_type == "RST":
                                # If the connection wasn't established and gets reset, mark as unsuccessful
                                for key in [conn_key, rev_conn_key]:
                                    if key in connections and not connections[key]["established"]:
                                        src_ip_original = connections[key]["src_ip"]
                                        second = int(connections[key]["first_syn_time"])
                                        if src_ip_original == legit_ip:
                                            unsuccessful_syns_legit[second] += 1
                                        elif src_ip_original == attack_ip:
                                ...
                                # Connection completed normally, clean up but don't mark as unsuccessful
                                for key in [conn_key, rev_conn_key]:
                                    if key in connections:
                                        del connections[key]
    ...
    # Only mark as unsuccessful those that received no SYN-ACK response
    for conn_key, data in list(connections.items()):
        if data["syn_count"] > 0 and not data.get("established", False):
            src_ip = data["src_ip"]
            second = int(data["first_syn_time"])
            
            # Only count as unsuccessful if we never got a SYN-ACK
            if not data.get("syn_ack_received", False):
                if src_ip == legit_ip:
                    unsuccessful_syns_legit[second] += 1
